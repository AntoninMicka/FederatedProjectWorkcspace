...
- [ ] [in progress] **F-M2-META-AI-01-B — Durable lokální návrh a validovaný diff (cílová úroveň: implemented).** Implementovat same-node roli, Context Manifest, striktní parser a node-local preview bez zápisu do projektu.
- [ ] [planned] **F-M2-META-AI-01-C — Potvrzený metadata zápis, desktop UI a akceptace (cílová úroveň: PoC validated).** Přidat bezpečný diff, výběr polí, jediný Workspace commit, recovery a chybové testy.
