...
**M1-03 — První Markdown artefakt a editor.** Navázat na vytvoření/otevření projektu: vymezit vytvoření a uložení Markdown artefaktu z UI přes Workspace. Před implementací uzavřít mutující kontrakt editoru podle V-10 (operation ID, retry/receipt, autorizace a souběh).
...
- [ ] [planned] **M1-03 — Markdown artefakt a editor (cílová úroveň: PoC validated).** Navrhnout minimální UI vytvoření/úpravy artefaktu, navázat existující Workspace a vymezit operation ID/retry/receipt pro mutující vstup dle V-10. Ověřit obsah i metadata z Gitu, restart a přerušený zápis; neprovádět ukládání pouze do paměti nebo SQLite projekce.
