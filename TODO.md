...
**M1-02 — Vytvoření a registrace projektu.** Navázat na V-08: připravit lokální node konfiguraci a založení projektu s commitnutým project.json tak, aby nebylo nutné ručně psát JSON. Před implementací vymezit transakční hranice registrace/inicializace, zachování ID a chování po přerušení. Editor a persistentní API mutace závisejí na kontraktu V-10.
...
- [ ] [planned] **M1-02 — Inicializace a registrace projektu (cílová úroveň: PoC validated).** Vymezit a implementovat explicitní vytvoření projektu/registrace v lokálním uzlu, včetně výchozí konfigurace, stabilních ID a chybových cest při částečném dokončení. Zachovat existující data a journal, nedovolit duplicitní registraci stejného root s jiným state. Navazuje na V-08; před novými persistentními API operacemi uzavřít příslušný kontrakt V-10.
...
- [ ] [planned] **V-08 — Aplikační integrace konfigurace (cílová úroveň: implemented).** Otevření nyní ověřuje project_id registrace, existenci a bezpečné umístění kořenů/stavu i verzi (M1-01). Zbývá vazba identity na credential úložiště. Navrhnout bezpečnou inicializaci a serializovaný zápis project.json přes Workspace a oddělený obnovitelný zápis node.json, včetně budoucích explicitních migrací. Před změnou popsat crash boundaries; samostatný validátor nic nezapisuje; M1-01 čte commitnutou konfiguraci při otevření, konfigurace není součástí indexu artefaktů.
