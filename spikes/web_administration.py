...
document.querySelector('#admin-mapping-create').onsubmit=event=>{event.preventDefault();const fields=new FormData(event.target);const project_ids=Array.from(document.querySelectorAll('#admin-mapping-projects input:checked'),i=>i.value).sort();adminChange({action:'propose-mapping',local_user_id:fields.get('local_user_id'),peer_node_id:fields.get('peer_node_id'),peer_user_id:fields.get('peer_user_id'),project_ids});};
document.querySelector('#admin-mapping-import').onsubmit=event=>{event.preventDefault();try{const envelope=JSON.parse(new FormData(event.target).get('envelope'));adminChange({action:'import-mapping',envelope});}catch(error){adminStatus.textContent='Potvrzení musí být platný JSON.';}};
'''
